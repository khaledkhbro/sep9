...
import { HelpCircle, Clock, Zap, CreditCard, AlertCircle } from "lucide-react"
...
interface SupportRequestDialogProps {
  ...
  onClose: () => void
  chatId?: string
  onRequestComplete?: (ticketData: any) => void
}

export function SupportRequestDialog({ isOpen, onClose, chatId, onRequestComplete }: SupportRequestDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("[v0] Creating support ticket from chat:", ticketData)

      // Simulate API call
      await new Promise((resolve) => setTimeout(resolve, 2000))

      if (formData.supportType === "priority") {
        toast.success("Priority support ticket created! Admin will respond within 1 hour.")
      } else {
        toast.success("Support ticket created! Admin will respond within 24-72 hours.")
      }

      if (onRequestComplete) {
        onRequestComplete(ticketData)
      }
          ...
          {formData.supportType === "priority" && (
            <div className="bg-orange-50 border border-orange-200 rounded-lg p-4">
              ...
              <p className="text-sm text-orange-700">
                ...
                with guaranteed admin response within 1 hour.
              </p>
            </div>
          )}
          ...
          {chatId && (
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-center gap-2 text-blue-800 mb-2">
                ...
                <span className="font-medium">Chat Context</span>
              </div>
              <p className="text-sm text-blue-700">
                This support request will be linked to your current chat conversation. Admin can see the chat history to
                better assist you.
              </p>
            </div>
          )}
